...
interface Props {
  ...
  bgMode?: "dark" | "white";
  /** Optional custom point color (overrides bgMode default). */
  pointColor?: string;
  /** Optional custom point radius multiplier. */
  radiusMul?: number;
}
 ...
 */
export const ScatterCanvas: React.FC<Props> = ({ points, plotAreaW, plotAreaH, pixelScale = 2, bgMode = "dark", pointColor, radiusMul = 1 }) => {
  ...
  useEffect(() => {
    ...
    const r  = Math.max(0.8, 1.4 * Math.min(sx, sy) * 0.5 * radiusMul);
    const TAU = Math.PI * 2;

    // Use custom color if provided, else dark blue for white mode, bright green for dark mode
    ctx.fillStyle = pointColor ?? (bgMode === "white" ? "rgba(25, 103, 210, 0.75)" : "rgba(74, 222, 128, 0.55)");
    ...
    ctx.fill();
  }, [points, plotAreaW, plotAreaH, w, h, bgMode, pointColor, radiusMul]);
